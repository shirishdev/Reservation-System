...
class HomepageController extends CI_Controller {
	...
	public function DisplayFetchservicedetails() {
		...
		$date = !empty($this->input->get('date')) ? date('Y-m-d',strtotime($this->input->get('date'))) : date('Y-m-d');
	...
	public function GetTimeSlot($date,$ServiceData) {
		...
		for($i=$startTs; $i < $endTs; $i +=$lengthTs) {
           $class = 'available';
            ...
            $CountResult=$this->Homepagemodel->ValidateBookingRecords($service_id,$date,$time);
           
           $Bookedcount = !empty($CountResult->totalcount) ? (int)$CountResult->totalcount : 0;
           //var_dump( $Bookedcount);
	...
	public function PartySizeslot() {
        ...
        $data['bookingCount']=$this->Homepagemodel->ValidateBookingRecords($service_id,$date,$timeslot);
